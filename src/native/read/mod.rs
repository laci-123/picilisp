...
fn whitespace_character(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "a whitespace"))(mem, input)
}
...
fn comment_start(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "a semicolon"))(mem, input)
}
...
fn number_sign(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "a '+' or a '-' character"))(mem, input)
}
...
fn digit(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "a digit"))(mem, input)
}
...
fn character_start(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "a % sign"))(mem, input)
}
...
fn escape_character(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "an escape character ('n', 'r', 't' or '\\'"))(mem, input)
}
...
fn symbol_char(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (satisfy(&any_character, &p, "any character except '(', ')' and whitespace characters"))(mem, input)
}
...
fn expression(mem: &mut Memory, input: StringWithLocation) -> Result<ParserOk, ParserError> {
    ...
    (one_of_these_two(&atom, &list))(mem, jnk.rest_of_input)
}
...
pub fn read(mem: &mut Memory, args: &[GcRef], _env: GcRef) -> NativeResult {
    let nr = validate_arguments(mem, READ.name, &vec![ParameterType::Any], args.clone());
    if nr.is_err() {
        return nr;
    }
    ...
    let result =
    match expression(mem, StringWithLocation { string: input, location: Location::Stdin { line: 1, column: 1 } }) {
        Ok(output) => {
            let mut vec = vec![("status", mem.symbol_for("ok")), ("result", output.value), ("rest", output.rest_of_input.string)];
            if let Some(line) = output.location.get_line() {
                vec.push(("line", fit_to_number(mem, line)));
            }
            if let Some(column) = output.location.get_column() {
        ...
        Err(err)   => {
            match err.kind {
                ParserErrorKind::Fatal => {
                    let vec = vec![("status", mem.symbol_for("error")), ("error", mem.symbol_for("not-implemented"))];
                    make_plist(mem, &vec)
                }
                ParserErrorKind::Unfinished => {
