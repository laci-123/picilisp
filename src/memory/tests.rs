...
fn mem_allocate_meta() {
    ...
    
    {
        ...
        assert_eq!(m1.get_metadata().unwrap().location.get_file(), None);
        assert_eq!(m1.get_metadata().unwrap().location.get_line().unwrap(), 23);
        assert_eq!(m1.get_metadata().unwrap().documentation, "");
        assert_eq!(m2.get_metadata().unwrap().location.get_file().unwrap(), PathBuf::from("~/the/input/file.lisp"));
        assert_eq!(m2.get_metadata().unwrap().location.get_column().unwrap(), 42);
